...
class CameraToBaseOffset(object):
    def __init__(self):
        ...
        self.camera_frame_id = rospy.get_param("~camera_frame_id", "left_camera_optical_frame")
    ...
    def source_odom_callback(self, msg):
        with self.lock:
            ...
            transformed_pose_cov_matrix = numpy.matrix(msg.pose.covariance).reshape(6, 6)
            rotation_matrix = htm_odom_base_to_base[:3, :3]
            transformed_pose_cov_matrix[:3, :3] = (rotation_matrix.T).dot(
                transformed_pose_cov_matrix[:3, :3].dot(rotation_matrix))
            transformed_pose_cov_matrix[3:6, 3:6] = (rotation_matrix.T).dot(
                transformed_pose_cov_matrix[3:6, 3:6].dot(rotation_matrix))
            ...
            transformed_odom_msg.pose.pose.position = Point(*list(htm_odom_base_to_base[:3, 3]))
            transformed_odom_msg.pose.pose.orientation = Quaternion(
                *list(tf.transformations.quaternion_from_matrix(htm_odom_base_to_base)))
            transformed_odom_msg.pose.covariance = numpy.array(transformed_pose_cov_matrix).reshape(-1,).tolist()

            # publish
            self.pub.publish(transformed_odom_msg)
            if self.publish_tf:
                broadcast_transform(self.broadcast, transformed_odom_msg, self.invert_tf)
