...
class CameraToBaseOffset(object):
    ...
    def source_odom_callback(self, msg):
        with self.lock:
            ...
            current_base_to_camera = self.calculate_base_to_sensor_transform()
            if self.initial_matrix != None:
                self.initial_matrix = current_base_to_camera
            camera_transformation = numpy.dot(numpy.linalg.inv(self.initial_matrix), current_base_to_camera)
            ...
            new_odom_matrix = camera_transformation.dot(source_odom_matrix)
            ...
            rotation_matrix = camera_transformation[:3, :3]
    ...
    def calculate_base_to_sensor_transform(self):
        try:
            (trans,rot) = self.listener.lookupTransform(self.base_link_frame, self.camera_frame, rospy.Time(0))
        except:
            rospy.logwarn("[%s] failed to solve tf in initialize_odometry: %s to %s", rospy.get_name(), self.base_link_frame, self.camera_frame)
            return None
        base_link_to_camera = self.make_homogeneous_matrix(trans, rot) # base_link -> camera
        return base_link_to_camera
