...
#include <jsk_robot_startup/max_velocity_map.h>
...
void MaxVelocityMap::amclCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
  ...
  if( map_ != NULL ) {
    ...
    float max_vel_ratio = std::max(map_->cells[index_x + index_y * map_->size_x].occ_state / 255.0, 0.2); // use std::max to avoid too slow movement
    // change max_vel parameters dynamically
...
map_t* MaxVelocityMap::convertMap( const nav_msgs::OccupancyGrid& map_msg )
{
  map_t* map = map_alloc();
  ROS_ASSERT(map);
  // Convert OccupancyGrid to map_t
