...
if [ -e $WORKSPACE_SPOT_DRIVER ]; then
    ...
    wstool merge -t . jsk-ros-pkg/jsk_robot/jsk_spot_robot/jsk_spot_startup/jsk_spot_driver.rosinstall -y -r
...
if [ -e $WORKSPACE_SPOT_CORAL ]; then
    ...
    wstool merge -t . jsk-ros-pkg/jsk_robot/jsk_spot_robot/jsk_spot_startup/jsk_spot_coral.rosinstall -y -r
    wstool update -t . -j 4 -m 60 --delete-changed-uris --continue-on-error
    catkin build -j4 --continue-on-failure
else
...
if [ -e $WORKSPACE_SPOT ]; then
    ...
    wstool merge -t . jsk-ros-pkg/jsk_robot/jsk_spot_robot/jsk_spot_startup/jsk_spot_user.rosinstall -y -r
    wstool update -t . -j 4 -m 60 --delete-changed-uris --continue-on-error
    catkin build -j4 --continue-on-failure
else
