    ...
    def update(self,
        ...
        with self.lock:
            if observed_time < self.last_observed:
                rospy.logerr(
                    'observed time is earlier than last observed time, dropped.')
                return
            ...
            if self.state == self.FOUND or self.state == self.MOVING or self.state == self.APPROCHING:
                self.velocity = (observed_position - self.position) / \
                    (observed_time - self.last_observed).to_sec()
            else:
                self.velocity = PyKDL.Vector()
...
class MultiObjectTracker(object):
    ...
    def __init__(self):
        ...
        self._threshold_velocity = rospy.get_param(
        ...
        while not rospy.is_shutdown():
            try:
                rospy.wait_for_message(
                    '~input_bbox_array', BoundingBoxArray, 3)
            ...
            rate.sleep()
        mf_sub_bbbox_array = message_filters.Subscriber(
            '~input_bbox_array', BoundingBoxArray)
        mf_sub_tracking_labels = message_filters.Subscriber(
            '~input_tracking_labels', LabelArray)
        ts = message_filters.ApproximateTimeSynchronizer(
            [mf_sub_bbbox_array, mf_sub_tracking_labels], 10, slop)
                ...
                if tracking_label.id not in self._dict_objects:
                    if len(self._dict_objects) < self._num_max_track:
                        self._dict_objects[tracking_label.id] = \
                            TrackedObject(
                                bbox.label,
                                convert_msg_point_to_kdl_vector(
                                    bbox.pose.position),
                                time_observed
                        )
                    else:
